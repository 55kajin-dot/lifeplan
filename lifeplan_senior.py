...
with t1:
    ...
    st.caption("生活費8項目と介護費は月額（万円/月）。それ以外（年収・一時収入・一時支出）は年額（万円）。年次（1年刻み）で計算します。※【おひとりさまの入力方法】:片方の現在年齢と死亡年齢を0に、収入も0にし、単身世帯になったときの生活費の割合(％)を100にすれば対応可能です。　※シニアが作りました。")
...
# ★もう1行分だけ下げるスペーサー（調整用）
    ...
    with L2:
        h_spends = build_lumps(
            ...
            "夫の一時支出（年額・万円）",
            DEFAULT["h_spend"],
            note_text="※車買換え、海外旅行、子の結婚費用、配偶者の葬式代や、その際の子への相続分の分配金など　※数値入力後、計算に反映させるため必ず右の使用ボタン欄を☑にしてください"
        )
    with R2:
